...
public class QuoteEditor
	...
	private boolean save()
		...
		if(q.author.isEmpty()) { alert("Empty Author");tfAuthor.requestFocus(); return false;}
		q.source=tfSource.getText().trim();
		q.date=tfDate.getText().trim();
		q.quoteSource=tfQuoteSource.getText().trim();
		if(q.quoteSource.isEmpty()) { alert("Empty Quote Source"); tfQuoteSource.requestFocus(); return false;}
		q.theQuote=quoteArea.getText().trim();
		if(q.theQuote.isEmpty()) { alert("Empty Quote"); quoteArea.requestFocus();return false;}
		String keywords= tfKeywords.getText().trim();
		if(keywords.isEmpty()) { alert("Empty Keywords"); return false;}
		...
		if(tags.isEmpty())
			{
			alert("Empty Keywords"); return false;
			}
			...
			if(!this.rdfFile.exists())
				...
				while(reader.hasNext())
					...
					switch(evt.getEventType())
						...
						case XMLEvent.START_ELEMENT:
							...
							if(next!=null && next.isEndElement())
								...
								w.writeEmptyElement(
										name.getPrefix(),
										name.getLocalPart(),
										e.getNamespaceURI(name.getPrefix())
										);
								reader.nextEvent();//consumme next event that must be endElement
								}
							else
								{
								w.writeStartElement(
						...
						default:throw new XMLStreamException("type not handled :"+evt.getClass()+" "+evt.getEventType());
						}
					}
				reader.close();
				w.flush();
